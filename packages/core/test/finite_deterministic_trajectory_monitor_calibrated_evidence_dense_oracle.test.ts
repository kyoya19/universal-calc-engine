...
function request(): FiniteDeterministicTrajectoryMonitorCalibratedEvidenceRequest {
  ...
  return {
    initialDistribution: [{ stateId: 'x', probability: 0.55 }, { stateId: 'y', probability: 0.45 }],
    horizon: 3,
    monitorStates,
    initialMonitorStateByHiddenState: [{ stateId: 'x', monitorStateId: 'q0' }, { stateId: 'y', monitorStateId: 'q1' }],
    monitorTransitionByStep: Array.from({ length: 3 }, (_, step) => monitorStates.flatMap((q) =>
    ...
    evidenceLikelihoods: [
      [{ stateId: 'x', likelihood: 0.9 }, { stateId: 'y', likelihood: 0.5 }],
      [{ stateId: 'x', likelihood: 0.2 }, { stateId: 'y', likelihood: 0.8 }],
      [{ stateId: 'x', likelihood: 0.75 }, { stateId: 'y', likelihood: 0.35 }],
      [{ stateId: 'x', likelihood: 0.6 }, { stateId: 'y', likelihood: 0.95 }]
    ]
  };
}
...
function denseOracle(req: FiniteDeterministicTrajectoryMonitorCalibratedEvidenceRequest) {
  const hidden = ['x', 'y'];
  ...
  const initialP = new Map(req.initialDistribution.map((entry) => [entry.stateId, entry.probability] as const));
  const evidence = req.evidenceLikelihoods.map((row) => new Map(row.map((entry) => [entry.stateId, entry.likelihood] as const)));
  const transition = model.transitions.map((entry) => ({ from: entry.from, to: entry.to, probability: Number(entry.probability) }));
  const delta = req.monitorTransitionByStep.map((row) => new Map(row.map((entry) => [
    `${entry.monitorStateId}:${entry.fromStateId}:${entry.toStateId}`,
    entry.nextMonitorStateId
  ] as const)));
  const forward: number[][][] = [];
  const first = Array.from({ length: hidden.length }, () => Array(monitor.length).fill(0) as number[]);
  for (const stateId of hidden) {
    first[hIndex.get(stateId)!]![qIndex.get(initialQ.get(stateId)!)!] = (initialP.get(stateId) ?? 0) * (evidence[0]!.get(stateId) ?? 0);
  }
  forward.push(first);
  for (let step = 0; step < req.horizon; step += 1) {
    const next = Array.from({ length: hidden.length }, () => Array(monitor.length).fill(0) as number[]);
  ...
  return { hidden, monitor, hIndex, qIndex, evidence, transition, delta, forward };
}

describe('Candidate AC independent raw-probability dense X-by-Q oracle', () => {
  it('matches dense forward evidence and monitor joint masses', () => {
    ...
    for (const q of dense.monitor) {
      ...
      const mass = dense.hidden.reduce((sum, stateId) => sum + final[dense.hIndex.get(stateId)!]![qi]!, 0);
      const actual = analysis.jointEvidenceMonitorDistribution?.find((entry) => entry.monitorStateId === q)?.jointProbability ?? 0;
  ...
  it('uses an independent dense backward array to reproduce combined smoothing', () => {
    ...
    const beta: number[][][] = Array.from({ length: req.horizon + 1 }, () =>
      Array.from({ length: dense.hidden.length }, () => Array(dense.monitor.length).fill(0) as number[])
    );
    for (const stateId of dense.hidden) {
      for (const q of dense.monitor) beta[req.horizon]![dense.hIndex.get(stateId)!]![dense.qIndex.get(q)!] = target.has(q) ? 1 : 0;
    }
      ...
      for (const fromStateId of dense.hidden) {
        for (const q of dense.monitor) {
          ...
          for (const edge of dense.transition.filter((entry) => entry.from === fromStateId)) {
            ...
            value += edge.probability * (dense.evidence[step + 1]!.get(edge.to) ?? 0) * beta[step + 1]![dense.hIndex.get(edge.to)!]![dense.qIndex.get(qNext)!]!;
          }
    ...
    let joint = 0;
    for (const stateId of dense.hidden) {
      for (const q of dense.monitor) if (target.has(q)) joint += final[dense.hIndex.get(stateId)!]![dense.qIndex.get(q)!]!;
    }
