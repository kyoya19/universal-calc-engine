...
function request(): FiniteDeterministicTrajectoryMonitorTransitionCalibratedEvidenceRequest {
  ...
  return {
    ...
    transitionEvidenceLikelihoodsByStep: [
      [
        { fromStateId: 'a', toStateId: 'a', likelihood: 0.3 },
        { fromStateId: 'a', toStateId: 'b', likelihood: 0.8 },
        { fromStateId: 'b', toStateId: 'a', likelihood: 0.55 },
        { fromStateId: 'b', toStateId: 'b', likelihood: 0.25 }
      ],
      [
        { fromStateId: 'a', toStateId: 'a', likelihood: 0.7 },
        { fromStateId: 'a', toStateId: 'b', likelihood: 0.2 },
        { fromStateId: 'b', toStateId: 'a', likelihood: 0.45 },
        { fromStateId: 'b', toStateId: 'b', likelihood: 0.9 }
      ],
      [
        { fromStateId: 'a', toStateId: 'a', likelihood: 0.6 },
        { fromStateId: 'a', toStateId: 'b', likelihood: 0.95 },
        { fromStateId: 'b', toStateId: 'a', likelihood: 0.35 },
        { fromStateId: 'b', toStateId: 'b', likelihood: 0.75 }
      ]
    ]
  };
}
...
function initialEvidence(
  req: FiniteDeterministicTrajectoryMonitorTransitionCalibratedEvidenceRequest,
  stateId: StateId
): number {
...
function pairEvidence(
  ...
  return req.transitionEvidenceLikelihoodsByStep[step - 1]!.find(
  ...
  const visit = (
    ...
    for (const transition of model.transitions.filter((entry) => entry.from === fromStateId)) {
      ...
      visit(
        ...
        evidenceWeight * pairEvidence(req, nextStep, fromStateId, transition.to),
        nextStep + 1
      );
    }
  };
...
function enumeratePrefix(
  ...
  const truncated: FiniteDeterministicTrajectoryMonitorTransitionCalibratedEvidenceRequest = {
    ...req,
    horizon: endStep,
    monitorTransitionByStep: req.monitorTransitionByStep.slice(0, endStep),
    transitionEvidenceLikelihoodsByStep: req.transitionEvidenceLikelihoodsByStep.slice(0, endStep)
  };
...
describe('Candidate AD independent complete-path and prefix enumeration oracles', () => {
  it('matches P(E), P(E,Q_T) and P(Q_T|E) from complete concrete-transition enumeration', () => {
    ...
    for (const path of paths) {
      ...
      byMonitor.set(
        monitorStateId,
        (byMonitor.get(monitorStateId) ?? 0) + path.baseProbability * path.evidenceWeight
      );
    }
        ...
        for (const monitorStateId of req.monitorStates) {
          const expected =
            selected
              .filter(
                (path) =>
                  path.states[step] === stateId && path.monitors[step] === monitorStateId
              )
              .reduce(
