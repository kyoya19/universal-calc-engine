...
function request(): FiniteDeterministicTrajectoryMonitorTransitionCalibratedEvidenceRequest {
  ...
  return {
    ...
    transitionEvidenceLikelihoodsByStep: [
      [
        { fromStateId: 'x', toStateId: 'x', likelihood: 0.2 },
        { fromStateId: 'x', toStateId: 'y', likelihood: 0.8 },
        { fromStateId: 'y', toStateId: 'x', likelihood: 0.65 },
        { fromStateId: 'y', toStateId: 'y', likelihood: 0.3 }
      ],
      [
        { fromStateId: 'x', toStateId: 'x', likelihood: 0.75 },
        { fromStateId: 'x', toStateId: 'y', likelihood: 0.35 },
        { fromStateId: 'y', toStateId: 'x', likelihood: 0.4 },
        { fromStateId: 'y', toStateId: 'y', likelihood: 0.95 }
      ],
      [
        { fromStateId: 'x', toStateId: 'x', likelihood: 0.6 },
        { fromStateId: 'x', toStateId: 'y', likelihood: 0.9 },
        { fromStateId: 'y', toStateId: 'x', likelihood: 0.25 },
        { fromStateId: 'y', toStateId: 'y', likelihood: 0.7 }
      ]
    ]
  };
}
...
function denseOracle(req: FiniteDeterministicTrajectoryMonitorTransitionCalibratedEvidenceRequest) {
  ...
  const pairEvidence = req.transitionEvidenceLikelihoodsByStep.map((row) => new Map(row.map((entry) => [
    `${entry.fromStateId}:${entry.toStateId}`,
    entry.likelihood
  ] as const)));
    ...
    for (const fromStateId of hidden) {
      for (const q of monitor) {
        ...
        for (const edge of transition.filter((entry) => entry.from === fromStateId)) {
          ...
          target[qi] =
            (target[qi] ?? 0) +
            mass * edge.probability * (pairEvidence[step]!.get(`${fromStateId}:${edge.to}`) ?? 0);
        }
      }
    }
    forward.push(next);
...
describe('Candidate AD independent raw-probability dense X-by-Q oracle', () => {
  it('matches dense forward evidence and monitor joint masses', () => {
    ...
    const analysis = requireAnalysis(
      analyzeFiniteDeterministicTrajectoryMonitorUnderTransitionCalibratedEvidence(model, req)
    );
    ...
    for (const q of dense.monitor) {
      ...
      const actual =
        analysis.jointEvidenceMonitorDistribution?.find((entry) => entry.monitorStateId === q)?.jointProbability ?? 0;
  ...
  it('uses an independent dense backward array to reproduce combined smoothing', () => {
    ...
    for (const stateId of dense.hidden) {
      for (const q of dense.monitor) {
        beta[req.horizon]![dense.hIndex.get(stateId)!]![dense.qIndex.get(q)!] = target.has(q) ? 1 : 0;
      }
    }
      ...
      for (const fromStateId of dense.hidden) {
        for (const q of dense.monitor) {
          ...
          for (const edge of dense.transition.filter((entry) => entry.from === fromStateId)) {
            ...
            value +=
              edge.probability *
              (dense.pairEvidence[step]!.get(`${fromStateId}:${edge.to}`) ?? 0) *
