...
type PreparedResult = { ok: true; prepared: Prepared } | FiniteAdditiveTrajectoryFunctionalCalibratedEvidenceFailure;
...
type ForwardResult = { ok: true; forward: ForwardInternal } | FiniteAdditiveTrajectoryFunctionalCalibratedEvidenceFailure;

type SafeAddResult = { ok: true; value: number } | FiniteAdditiveTrajectoryFunctionalCalibratedEvidenceFailure;
...
function probabilityView(logProbability: number): Pick<AdditiveCalibratedEvidenceProbabilityAtom, 'probability' | 'logProbability' | 'probabilityUnderflowed'> {
  ...
  return {
    probability,
    logProbability,
    probabilityUnderflowed: probability === null
  };
}
...
function mapAaFailure(source: { failure: { code: string; message: string; path?: string; step?: number; stateId?: StateId; fromStateId?: StateId; toStateId?: StateId; valueTicks?: number; actual?: number; expected?: number; tolerance?: number } }): FiniteAdditiveTrajectoryFunctionalCalibratedEvidenceFailure {
  ...
  if (sourceCode === 'unsafe_additive_cumulative_tick_value') {
    code = 'unsafe_additive_calibrated_evidence_cumulative_tick_value';
  } else if (sourceCode === 'additive_support_limit_exceeded' || sourceCode === 'invalid_additive_support_limit') {
    code = 'additive_calibrated_evidence_support_limit_exceeded';
  } else if (sourceCode.includes('horizon')) {
    code = 'invalid_additive_calibrated_evidence_horizon';
  } else if (sourceCode.includes('tick')) {
    code = 'invalid_additive_calibrated_evidence_tick_value';
  }
  return failure(code, source.failure.message, {
    path: source.failure.path,
    step: source.failure.step,
    stateId: source.failure.stateId,
    fromStateId: source.failure.fromStateId,
    toStateId: source.failure.toStateId,
    valueTicks: source.failure.valueTicks,
    actual: source.failure.actual,
    expected: source.failure.expected,
    tolerance: source.failure.tolerance,
    sourceFailureCode: sourceCode
  });
}
...
function resolveOptions(
  ...
  horizon: number
): ResolvedOptions | FiniteAdditiveTrajectoryFunctionalCalibratedEvidenceFailure {
  ...
  if (!Number.isFinite(probabilityTolerance) || probabilityTolerance <= 0) {
    return failure('invalid_additive_calibrated_evidence_joint_request', 'probabilityTolerance must be a finite positive number', {
  ...
  if (!Number.isFinite(pairwiseConsistencyTolerance) || pairwiseConsistencyTolerance <= 0) {
    return failure('invalid_additive_calibrated_evidence_joint_request', 'pairwiseConsistencyTolerance must be a finite positive number', {
  ...
  if (!Number.isFinite(expectedCountTolerance) || expectedCountTolerance <= 0) {
    return failure('invalid_additive_calibrated_evidence_joint_request', 'expectedCountTolerance must be a finite positive number', {
  ...
  if (!Number.isInteger(maxHorizon) || maxHorizon < 0) {
    return failure('invalid_additive_calibrated_evidence_horizon', 'maxHorizon must be a non-negative integer', {
  ...
  if (!Number.isInteger(maxSupportSize) || maxSupportSize <= 0) {
    return failure('additive_calibrated_evidence_support_limit_exceeded', 'maxSupportSize must be a positive integer', {
...
function canonicalizeEvidenceRows(
  ...
  stateIds: StateId[],
  horizon: number
): { rows: CalibratedEvidenceLikelihoodEntry[][]; maps: Array<Map<StateId, number>> } | FiniteAdditiveTrajectoryFunctionalCalibratedEvidenceFailure {
  ...
  for (let step = 0; step < evidenceLikelihoods.length; step += 1) {
    ...
    if (!Array.isArray(row)) {
      return failure('invalid_additive_calibrated_evidence_likelihood_row', `evidenceLikelihoods[${step}] must be an array`, {
    ...
    for (let index = 0; index < row.length; index += 1) {
      ...
      if (entry === undefined || typeof entry.stateId !== 'string') {
        return failure('invalid_additive_calibrated_evidence_likelihood_row', `evidenceLikelihoods[${step}][${index}] requires string stateId`, {
          path: `request.evidenceLikelihoods[${step}][${index}].stateId`, step
        });
      }
      if (!known.has(entry.stateId)) {
        return failure('unknown_additive_calibrated_evidence_likelihood_state', `Unknown evidence state: ${entry.stateId}`, {
          path: `request.evidenceLikelihoods[${step}][${index}].stateId`, step, stateId: entry.stateId
        });
      }
      if (seen.has(entry.stateId)) {
        return failure('duplicate_additive_calibrated_evidence_likelihood_state', `Duplicate evidence state: ${entry.stateId}`, {
          path: `request.evidenceLikelihoods[${step}][${index}].stateId`, step, stateId: entry.stateId
        });
      }
      if (!Number.isFinite(entry.likelihood) || entry.likelihood < 0 || entry.likelihood > 1) {
        return failure('invalid_additive_calibrated_evidence_likelihood', 'Calibrated evidence likelihood must be a finite absolute probability in [0,1]', {
    ...
    for (const stateId of stateIds) {
      if (!seen.has(stateId)) {
        return failure('missing_additive_calibrated_evidence_likelihood_state', `Missing evidence state: ${stateId}`, {
...
function prepare(
  ...
  if (!Number.isSafeInteger(request.horizon) || request.horizon < 0) {
    return failure('invalid_additive_calibrated_evidence_horizon', 'horizon must be a non-negative safe integer', {
  ...
  if (request.horizon > resolved.maxHorizon) {
    return failure('invalid_additive_calibrated_evidence_horizon', `horizon ${request.horizon} exceeds maxHorizon ${resolved.maxHorizon}`, {
      path: 'request.horizon', actual: request.horizon, expected: resolved.maxHorizon
    });
  }
  if (!Array.isArray(request.evidenceLikelihoods) || request.evidenceLikelihoods.length !== request.horizon + 1) {
    return failure('additive_calibrated_evidence_length_mismatch', `evidenceLikelihoods must contain exactly horizon+1=${request.horizon + 1} rows`, {
      path: 'request.evidenceLikelihoods',
      actual: Array.isArray(request.evidenceLikelihoods) ? request.evidenceLikelihoods.length : -1,
      expected: request.horizon + 1
    });
  }
  ...
  const stateIds = model.states.map((state) => state.id).sort(compareStrings);
  const evidence = canonicalizeEvidenceRows(request.evidenceLikelihoods, stateIds, request.horizon);
  if ('ok' in evidence) return evidence;
  const effective = buildEffectiveEdges(model);
  const initialValueByState = new Map(aa.initialValueByState.map((entry) => [entry.stateId, entry.valueTicks] as const));
  const transitionValuesByStep = aa.transitionValueByStep.map(
    (row) => new Map(row.map((entry) => [pairKey(entry.fromStateId, entry.toStateId), entry.valueTicks] as const))
  );
  const initialDistribution = new Map<StateId, number>();
  for (const stateId of stateIds) initialDistribution.set(stateId, 0);
  ...
  return {
    ...
    prepared: {
      ...
      transitionValuesByStep,
      evidenceRows: evidence.rows,
      evidenceMaps: evidence.maps,
      initialDistribution,
      resolved,
...
function conditionalJointDistribution(
  ...
  for (const stateId of stateIds) {
    ...
    for (const [valueTicks, logMass] of entries) {
      const logProbability = logMass - logNormalizer;
...
function countUnderflow(
  atoms: Array<{ probabilityUnderflowed: boolean }>
): number {
  return atoms.reduce((sum, atom) => sum + (atom.probabilityUnderflowed ? 1 : 0), 0);
}
...
function runForward(
  ...
  for (let step = 0; step <= request.horizon; step += 1) {
    ...
    if (currentSupportSize > prepared.resolved.maxSupportSize) {
      return failure('additive_calibrated_evidence_support_limit_exceeded', `Joint evidence state-value support exceeds maxSupportSize at step ${step}`, {
    ...
    if (logEvidence === Number.NEGATIVE_INFINITY) {
      return {
        ...
        forward: {
          ...
          supportAtomUnderflowCount
        }
      };
    }
    if (!Number.isFinite(logEvidence) || logEvidence > prepared.resolved.probabilityTolerance * 10) {
      return failure('additive_calibrated_evidence_forward_mass_conservation_violation', 'Evidence prefix mass is invalid', {
        step, actual: Math.exp(logEvidence), expected: 1, tolerance: prepared.resolved.probabilityTolerance * 10
      });
    }
    if (step > 0 && logEvidence - previousLogEvidence > prepared.resolved.probabilityTolerance * 20) {
      return failure('additive_calibrated_evidence_forward_mass_conservation_violation', 'Evidence prefix probability increased after adding calibrated evidence', {
        step, actual: logEvidence, expected: previousLogEvidence, tolerance: prepared.resolved.probabilityTolerance * 20
      });
    }
    const jointStateValueDistribution = conditionalJointDistribution(prepared.stateIds, current, logEvidence);
    const valueDistribution = conditionalValueDistribution(prepared.stateIds, current, logEvidence);
    supportAtomUnderflowCount += countUnderflow(jointStateValueDistribution) + countUnderflow(valueDistribution);
      ...
      for (const [valueTicks, logMass] of current.get(fromStateId)?.entries() ?? []) {
        for (const edge of prepared.effectiveEdgesByState.get(fromStateId) ?? []) {
          ...
          if (increment === undefined) {
            return failure('internal_additive_calibrated_evidence_joint_structural_inconsistency', 'Missing canonical additive transition value', {
              step: step + 1, fromStateId: edge.from, toStateId: edge.to
            });
          }
          ...
          addLogMass(
            next.get(edge.to)!,
            nextValue.value,
            logMass + Math.log(edge.probability) + Math.log(likelihood)
          );
        }
      }
    }
  ...
  return {
    ...
    forward: {
      alphas,
      trajectory,
      evidenceLogProbability: previousLogEvidence,
...
function makeDiagnostics(
  ...
  return {
    ...
    evidenceProbabilityUnderflowed: logEvidence !== null && directProbability(logEvidence) === null,
...
function finalJointAggregate(
  prepared: Prepared,
  forward: ForwardInternal
): AdditiveCalibratedEvidenceJointAggregateAtom[] {
  const logEvidence = forward.evidenceLogProbability!;
  ...
  return [...byValue.entries()]
    .sort(([left], [right]) => left - right)
    .map(([valueTicks, logJointProbability]) => {
...
export function analyzeFiniteAdditiveTrajectoryFunctionalUnderCalibratedEvidence(
  ...
  if (forward.evidenceLogProbability === null) {
    return {
      ...
      finalEvidenceConditionedAggregateDistribution: null,
      jointEvidenceAggregateDistribution: null,
      diagnostics
    };
  }
  const evidenceProbability = directProbability(forward.evidenceLogProbability);
  const joint = finalJointAggregate(prepared, forward);
  const finalConditional = joint.map((atom) => ({
  ...
  return {
    ...
    finalEvidenceConditionedAggregateDistribution: finalConditional,
    jointEvidenceAggregateDistribution: joint,
    diagnostics
  };
}

function targetUnconditional(
  prepared: Prepared,
  targetValueTicks: number
): { possible: boolean; probability: number | null; logProbability: number | null } {
...
function targetJointLogMass(
  ...
  for (const stateId of prepared.stateIds) {
    result = logAddExp(result, final.get(stateId)?.get(targetValueTicks) ?? Number.NEGATIVE_INFINITY);
  }
  return result;
}
    ...
    for (const fromStateId of prepared.stateIds) {
      ...
      for (const valueTicks of forward.alphas[step]!.get(fromStateId)?.keys() ?? []) {
        ...
        for (const edge of prepared.effectiveEdgesByState.get(fromStateId) ?? []) {
          ...
          const increment = transitionValues.get(pairKey(edge.from, edge.to));
          if (increment === undefined) {
            return failure('internal_additive_calibrated_evidence_joint_structural_inconsistency', 'Missing transition increment in backward pass', {
              step: step + 1, fromStateId: edge.from, toStateId: edge.to
            });
          }
          const nextValue = checkedSafeAdd(valueTicks, increment, step + 1, edge.from, edge.to);
          if (!nextValue.ok) return nextValue;
          const nextLog = betas[step + 1]!.get(edge.to)?.get(nextValue.value);
          if (nextLog === undefined) continue;
          logProbability = logAddExp(
            logProbability,
            Math.log(edge.probability) + Math.log(likelihood) + nextLog
          );
        }
...
function impossibleConditioningResult(
  ...
  const evidenceProbability = evidencePossible ? directProbability(forward.evidenceLogProbability!) : 0;
  const diagnostics: FiniteAdditiveTrajectoryFunctionalCalibratedEvidenceConditioningDiagnostics = {
...
export function conditionFiniteAdditiveTrajectoryFunctionalOnCalibratedEvidenceAndExactValue(
  ...
  if (forward.evidenceLogProbability === null) {
    return impossibleConditioningResult(request, prepared, forward, aggregate, 'evidence');
  }
  if (!aggregate.possible) {
    return impossibleConditioningResult(request, prepared, forward, aggregate, 'aggregate');
  }
  const jointLog = targetJointLogMass(prepared, forward, request.targetValueTicks);
  if (jointLog === Number.NEGATIVE_INFINITY) {
    return impossibleConditioningResult(request, prepared, forward, aggregate, 'joint');
  }
  if (!Number.isFinite(jointLog)) {
    return failure('non_finite_additive_calibrated_evidence_joint_result', 'Joint evidence/aggregate log probability is invalid', {
      valueTicks: request.targetValueTicks, actual: jointLog
    });
  }
  ...
  for (let step = 0; step < request.horizon; step += 1) {
    ...
    for (const fromStateId of prepared.stateIds) {
      for (const [valueTicks, alphaLog] of forward.alphas[step]!.get(fromStateId)?.entries() ?? []) {
        for (const edge of prepared.effectiveEdgesByState.get(fromStateId) ?? []) {
          ...
          const term = alphaLog + Math.log(edge.probability) + Math.log(likelihood) + betaLog;
    ...
    for (const stateId of prepared.stateIds) {
      const row = pairwiseDistribution
        .filter((entry) => entry.fromStateId === stateId)
        .reduce((sum, entry) => sum + entry.probability, 0);
      const column = pairwiseDistribution
      ...
      if (
        Math.abs(row - expectedRow) > prepared.resolved.pairwiseConsistencyTolerance ||
        Math.abs(column - expectedColumn) > prepared.resolved.pairwiseConsistencyTolerance
      ) {
  ...
  const evidenceProbability = directProbability(evidenceLog);
  const jointEventProbability = directProbability(jointLog);
  const conditionalLog = jointLog - evidenceLog;
  const targetConditionalProbabilityGivenEvidence = directProbability(conditionalLog);
  const diagnostics: FiniteAdditiveTrajectoryFunctionalCalibratedEvidenceConditioningDiagnostics = {
    ...makeDiagnostics(prepared, forward),
    conditioningMethod: 'exact_joint_evidence_aggregate_log_forward_backward',
