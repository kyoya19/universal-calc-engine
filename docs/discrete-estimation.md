...
No prior distribution is applied and no Bayesian posterior is computed.
...
If multiple candidate values have the same best log-likelihood score within the current numerical tie tolerance, all are returned in `bestCandidateValues` and `estimatedValue` is `null` rather than choosing one arbitrarily.
